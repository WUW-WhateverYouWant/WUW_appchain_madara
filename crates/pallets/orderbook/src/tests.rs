...
frame_support::construct_runtime!(
	pub enum Test
	{
		...
		Orderbook: pallet_orderbook::{Pallet, Call, Storage, Event<T>, ValidateUnsigned},
	}
);
...
fn it_aggregates_the_price() {
	sp_io::TestExternalities::default().execute_with(|| {
		assert_eq!(Orderbook::average_price(), None);

		assert_ok!(Orderbook::submit_price(RuntimeOrigin::signed(test_pub()), 27));
		assert_eq!(Orderbook::average_price(), Some(27));

		assert_ok!(Orderbook::submit_price(RuntimeOrigin::signed(test_pub()), 43));
		assert_eq!(Orderbook::average_price(), Some(35));
	});
}
...
fn should_make_http_call_and_parse_result() {
	...
	t.execute_with(|| {
		// when
		let price = Orderbook::fetch_price().unwrap();
		// then
...
fn knows_how_to_mock_several_http_calls() {
	...
	t.execute_with(|| {
		let price1 = Orderbook::fetch_price().unwrap();
		let price2 = Orderbook::fetch_price().unwrap();
		let price3 = Orderbook::fetch_price().unwrap();
...
fn should_submit_signed_transaction_on_chain() {
	...
	t.execute_with(|| {
		// when
		Orderbook::fetch_price_and_send_signed().unwrap();
		// then
		let tx = pool_state.write().transactions.pop().unwrap();
		assert!(pool_state.read().transactions.is_empty());
		let tx = Extrinsic::decode(&mut &*tx).unwrap();
		assert_eq!(tx.signature.unwrap().0, 0);
		assert_eq!(tx.call, RuntimeCall::Orderbook(crate::Call::submit_price { price: 15523 }));
	});
}
...
fn should_submit_unsigned_transaction_on_chain_for_any_account() {
	...
	t.execute_with(|| {
		// when
		Orderbook::fetch_price_and_send_unsigned_for_any_account(1).unwrap();
		// then
		let tx = pool_state.write().transactions.pop().unwrap();
		let tx = Extrinsic::decode(&mut &*tx).unwrap();
		assert_eq!(tx.signature, None);
		if let RuntimeCall::Orderbook(crate::Call::submit_price_unsigned_with_signed_payload {
			price_payload: body,
			signature,
		}) = tx.call
		{
...
fn should_submit_unsigned_transaction_on_chain_for_all_accounts() {
	...
	t.execute_with(|| {
		// when
		Orderbook::fetch_price_and_send_unsigned_for_all_accounts(1).unwrap();
		// then
		let tx = pool_state.write().transactions.pop().unwrap();
		let tx = Extrinsic::decode(&mut &*tx).unwrap();
		assert_eq!(tx.signature, None);
...
fn should_submit_raw_unsigned_transaction_on_chain() {
	...
	t.execute_with(|| {
		// when
		Orderbook::fetch_price_and_send_raw_unsigned(1).unwrap();
		// then
		let tx = pool_state.write().transactions.pop().unwrap();
		assert!(pool_state.read().transactions.is_empty());
		let tx = Extrinsic::decode(&mut &*tx).unwrap();
		...
		assert_eq!(
			tx.call,
			RuntimeCall::Orderbook(crate::Call::submit_price_unsigned {
				block_number: 1,
				price: 15523
			})
		);
...
fn parse_price_works() {
	...
	for (json, expected) in test_data {
		assert_eq!(expected, Orderbook::parse_price(json));
	}
}