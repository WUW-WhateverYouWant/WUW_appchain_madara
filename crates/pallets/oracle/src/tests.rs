...
fn it_aggregates_the_price() {
	sp_io::TestExternalities::default().execute_with(|| {
		assert_eq!(Oracle::average_price(), None);

		assert_ok!(Oracle::submit_price(RuntimeOrigin::signed(test_pub()), 27));
		assert_eq!(Oracle::average_price(), Some(27));

		assert_ok!(Oracle::submit_price(RuntimeOrigin::signed(test_pub()), 43));
		assert_eq!(Oracle::average_price(), Some(35));
	});
}
...
fn should_make_http_call_and_parse_result() {
	...
	t.execute_with(|| {
		// when
		let price = Oracle::fetch_price().unwrap();
		// then
...
fn knows_how_to_mock_several_http_calls() {
	...
	t.execute_with(|| {
		let price1 = Oracle::fetch_price().unwrap();
		let price2 = Oracle::fetch_price().unwrap();
		let price3 = Oracle::fetch_price().unwrap();
...
fn should_submit_signed_transaction_on_chain() {
	...
	t.execute_with(|| {
		// when
		Oracle::fetch_price_and_send_signed().unwrap();
		// then
		let tx = pool_state.write().transactions.pop().unwrap();
		assert!(pool_state.read().transactions.is_empty());
		let tx = Extrinsic::decode(&mut &*tx).unwrap();
		assert_eq!(tx.signature.unwrap().0, 0);
		assert_eq!(tx.call, RuntimeCall::Oracle(crate::Call::submit_price { price: 15523 }));
	});
}
...
fn should_submit_unsigned_transaction_on_chain_for_any_account() {
	...
	t.execute_with(|| {
		// when
		Oracle::fetch_price_and_send_unsigned_for_any_account(1).unwrap();
		// then
		let tx = pool_state.write().transactions.pop().unwrap();
		let tx = Extrinsic::decode(&mut &*tx).unwrap();
		assert_eq!(tx.signature, None);
		if let RuntimeCall::Oracle(crate::Call::submit_price_unsigned_with_signed_payload {
			price_payload: body,
			signature,
		}) = tx.call
		{
...
fn should_submit_unsigned_transaction_on_chain_for_all_accounts() {
	...
	t.execute_with(|| {
		// when
		Oracle::fetch_price_and_send_unsigned_for_all_accounts(1).unwrap();
		// then
		let tx = pool_state.write().transactions.pop().unwrap();
		let tx = Extrinsic::decode(&mut &*tx).unwrap();
		assert_eq!(tx.signature, None);
...
fn should_submit_raw_unsigned_transaction_on_chain() {
	...
	t.execute_with(|| {
		// when
		Oracle::fetch_price_and_send_raw_unsigned(1).unwrap();
		// then
		let tx = pool_state.write().transactions.pop().unwrap();
		assert!(pool_state.read().transactions.is_empty());
		let tx = Extrinsic::decode(&mut &*tx).unwrap();
		...
		assert_eq!(
			tx.call,
			RuntimeCall::Oracle(crate::Call::submit_price_unsigned {
				block_number: 1,
				price: 15523
			})
		);
